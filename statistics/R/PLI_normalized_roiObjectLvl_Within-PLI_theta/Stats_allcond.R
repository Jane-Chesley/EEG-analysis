...
filtered_folders <- all_folders[grep("PLI_allcond", all_folders, fixed = TRUE)]
...
for (i in 1:nrow(all_data)) {
  ...
  # Variable 3
  configuration_labels <- c("normal", "scramble")
  # Replace numerical values with character labels
  long_data$Configuration <- category_labels[long_data$Configuration]
  
  
  # Convert input data from long format to wide format 
  wide_data <- pivot_wider(long_data, names_from = c(Species, Category, Configuration), values_from = Measurement)
  ...
  summary_stats <- long_data %>%
    group_by(Species, Category, Configuration) %>%
  ...
  results <- anova_test(
    ...
    within = c(Species, Category, Configuration)
  )
