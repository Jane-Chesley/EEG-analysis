...
for (i in 1:nrow(all_data)) {
  ...
  print(summary(transformedDataRM))
  cat("Pausing script. Was the transformation done correctly? Enter 'true' or 'false': ") # Pause the script to inspect data and get user input
  user_input1 <- readline()
  
  # Save the input to a data frame
  # input_df <- data.frame(Input = user_logical_input)
  # trueprint(input_df)
  ...
  # inspect the transformed data 
  print(summary(transformedDataME))
  cat("Pausing script. Was the transformation done correctly? Enter 'true' or 'false': ") # Pause the script to inspect data and get user input
  ...
  # sheet2 includes true/false, indicating whether the ART was done correctly 
  user_inputs <- data.frame(Transformation1 = user_input1, Transformation2 = user_input2)
  write.xlsx(user_inputs, "ART_output.xlsx", sheetName = "Transformation_Check", append = TRUE)
  
  # sheet3 includes results of rmAVOVA
  write.xlsx(rmANOVA_result, "ART_output.xlsx", sheetName = "rmANOVA_ART_result", append = TRUE)
  
  # sheet4 includes results of Mixed Effects Model
